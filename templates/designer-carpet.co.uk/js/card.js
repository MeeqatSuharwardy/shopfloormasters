window.cardJS=function(){
	...
	on('set','.product.card',function(d){var p=d.value;if(!p.lookup){return};var t=e(this);
		e.ajax('get','/api/products',{sku:p.sku,context:'card'},function(x,d){
			if(x.status==200){
				...
				if(p.thumbnail){p.thumbnail="https://cdn.designer-carpet.co.uk"+p.thumbnail}
				if(p.cut){
	...
	on('set','.product.card',function(d){var p=d.value;if(p.lookup){return false};var t=e(this);
		...
		if(p.thumbnail){
			...
			t.find("[data-reflect='thumbnail']").val("/images/coming_soon.png");
		};

		//Size...
		if(p.accessory&&!p.antislip||p.carpet){
			t.find('.size').hide();
		...
		//Saving...
		if(p.carpet||p.quote_only||!(p.retail||p.retail_sqm)){
			t.find(".saving").hide();
		...
		//Price sqm...
		if(p.rug||(p.accessory&&!p.underlay)){
